"""
Module làm sạch dữ liệu cho ngành bán lẻ
Hỗ trợ: CSV và Excel (XLSX/XLS)
Chức năng: loại bỏ trùng, thêm dữ liệu thiếu, chuẩn hóa định dạng
"""
...
class RetailDataCleaner:
    ...
    def load_file(self, file_path: str) -> pd.DataFrame:
        """
        Load file (CSV hoặc Excel) với xử lý encoding
        ...
        if suffix == '.csv':
            ...
            raise ValueError(f"Định dạng file không được hỗ trợ: {suffix}. Chỉ hỗ trợ CSV và Excel.")
    
    def _load_csv(self, file_path: str, encoding: str = 'utf-8') -> pd.DataFrame:
        ...
        for enc in encodings_to_try:
            try:
                ...
                logger.info(f"Đã load {len(self.df)} dòng từ CSV {file_path} (encoding: {enc})")
                return self.df
        ...
        logger.info(f"Đã load {len(self.df)} dòng từ CSV {file_path} (encoding: utf-8 with ignore)")
        return self.df
    
    def _load_excel(self, file_path: str) -> pd.DataFrame:
        """Load Excel file (.xlsx, .xls)"""
        try:
            # Thử đọc file với openpyxl engine
            self.df = pd.read_excel(file_path, engine='openpyxl')
            logger.info(f"Đã load {len(self.df)} dòng từ Excel {file_path}")
            return self.df
        except Exception as e1:
            logger.warning(f"Không thể đọc Excel với openpyxl: {e1}")
            try:
                # Fallback sang xlrd cho file .xls cũ
                self.df = pd.read_excel(file_path, engine='xlrd')
                logger.info(f"Đã load {len(self.df)} dòng từ Excel {file_path} (xlrd)")
                return self.df
            except Exception as e2:
                logger.error(f"Không thể đọc file Excel: {e2}")
                raise
    ...
    def calculate_derived_fields(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Tính lợi nhuận gộp theo giao dịch
        ...
        if 'thoi_gian_theo_giao_dich' in df.columns:
            sample_val = df['thoi_gian_theo_giao_dich'].dropna().iloc[0] if not df['thoi_gian_theo_giao_dich'].dropna().empty else None
            if sample_val and len(str(sample_val)) > 10:
                time_col = 'thoi_gian_theo_giao_dich'
                logger.info(f"Sử dụng cột '{time_col}' cho datetime fields")
        
        if time_col is None and 'thoi_gian' in df.columns:
            time_col = 'thoi_gian'
            logger.info(f"Sử dụng cột '{time_col}' cho datetime fields")
        
        if time_col and time_col in df.columns:
            try:
                df['ngay'] = pd.to_datetime(df[time_col], dayfirst=True).dt.date
                df['thang'] = pd.to_datetime(df[time_col], dayfirst=True).dt.to_period('M').astype(str)
                df['nam'] = pd.to_datetime(df[time_col], dayfirst=True).dt.year
                df['tuan'] = pd.to_datetime(df[time_col], dayfirst=True).dt.isocalendar().week
                df['thu_trong_tuan'] = pd.to_datetime(df[time_col], dayfirst=True).dt.day_name()
                df['gio'] = pd.to_datetime(df[time_col], dayfirst=True).dt.hour
                
                logger.info(f"Đã tạo datetime fields từ cột '{time_col}'")
                logger.info(f"Khoảng thờigian: {df['ngay'].min()} đến {df['ngay'].max()}")
            except Exception as e:
                logger.warning(f"Lỗi khi parse datetime từ '{time_col}': {e}")
                df['ngay'] = pd.Timestamp.now().date()
                df['thang'] = pd.Timestamp.now().to_period('M').astype(str)
                df['nam'] = pd.Timestamp.now().year
                df['tuan'] = pd.Timestamp.now().isocalendar().week
                df['thu_trong_tuan'] = pd.Timestamp.now().day_name()
                df['gio'] = pd.Timestamp.now().hour
        
        return df
    
    def clean(self, file_path: str, remove_duplicates: bool = True, 
              fill_missing: bool = True) -> pd.DataFrame:
        """Pipeline làm sạch dữ liệu chính - Hỗ trợ cả CSV và Excel"""
        logger.info(f"Bắt đầu làm sạch file: {file_path}")
        
        # Load dữ liệu (tự động nhận diện định dạng)
    ...
    def save_cleaned(self, output_path: str, format: str = 'csv'):
        """
        ...
        elif format.lower() in ['excel', 'xlsx', '.xlsx', 'xls', '.xls']:
...
def clean_file(input_path: str, output_path: str) -> pd.DataFrame:
    """Làm sạch file (CSV hoặc Excel) và lưu kết quả"""
    cleaner = RetailDataCleaner()
    df = cleaner.clean(input_path)
    
    # Tự động nhận diện định dạng output từ extension
    output_ext = Path(output_path).suffix.lower()
    if output_ext in ['.xlsx', '.xls']:
        cleaner.save_cleaned(output_path, format='excel')
    else:
        cleaner.save_cleaned(output_path, format='csv')
...
if __name__ == '__main__':
    ...
    if len(sys.argv) < 3:
        ...
        print("Hỗ trợ: .csv, .xlsx, .xls")
        sys.exit(1)
    
    input_file = sys.argv[1]
    output_file = sys.argv[2]
    
    df = clean_file(input_file, output_file)
