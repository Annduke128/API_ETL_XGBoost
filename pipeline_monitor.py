...
class PipelineMonitor:
    ...
    def _get_clickhouse_engine(self):
        """Kết nối ClickHouse"""
        try:
            ...
            return Client(host=host, port=port, database=db, user=user, password=password)
        except Exception as e:
            logger.error(f"Không thể kết nối ClickHouse: {e}")
            return None
    ...
    def log_sync_stage(self):
        ...
        try:
            ...
            # ClickHouse counts
            ch_counts = {}
            ch_client = self._get_clickhouse_engine()
            
            if ch_client:
                try:
                    for table in ['products', 'transactions', 'transaction_details', 'branches']:
                        try:
                            result = ch_client.execute(f"SELECT COUNT() FROM {table}")
    ...
    def log_dbt_stage(self):
        ...
        try:
                ...
                try:
                    ...
                    try:
                        if 'daily' in table or 'sales' in table:
                            date_result = ch_client.execute(f"""
            ...
            try:
                result = ch_client.execute("SELECT COUNT() FROM fct_regular_sales")
                ...
                if ml_ready_count > 0:
                    ...
                    try:
                        prod_result = ch_client.execute("SELECT COUNT(DISTINCT product_code) FROM fct_regular_sales")
                        n_products = prod_result[0][0] if prod_result else 0
                        
                        branch_result = ch_client.execute("SELECT COUNT(DISTINCT branch_code) FROM fct_regular_sales")
    ...
    def log_forecast_stage(self):
        ...
        try:
            ...
            try:
                result = ch_client.execute("SELECT COUNT() FROM forecasts")
